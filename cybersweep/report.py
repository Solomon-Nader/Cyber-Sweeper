...
from cybersweep import __version__
from cybersweep.models import SEVERITY_ORDER, Host, Port, ScanResult
from cybersweep.utils import human_duration
...
def render_text(result: ScanResult, flt: Optional[ReportFilter] = None,
    ...
    out: list[str] = []
    title = f"CyberSweep report - scan #{result.scan_id}" if result.scan_id else "CyberSweep report"
    out.append(title)
...
def render_markdown(result: ScanResult, flt: Optional[ReportFilter] = None) -> str:
    flt = flt or ReportFilter()
    hosts = flt.apply(result)
    out = [f"# CyberSweep report{f' - scan #{result.scan_id}' if result.scan_id else ''}", ""]
...
def render_html(result: ScanResult, flt: Optional[ReportFilter] = None) -> str:
    ...
    parts = [
        ...
        f"<title>CyberSweep report {f'#{result.scan_id}' if result.scan_id else ''}</title>",
        f"<style>{_CSS}</style></head><body>",
        "<header><h1>CyberSweep Network Inspection Report</h1>",
    ...
    parts.append(f"</main><footer>Generated by CyberSweep {__version__}. Scan only networks you are authorised to test.</footer></body></html>")
    return "".join(parts)
