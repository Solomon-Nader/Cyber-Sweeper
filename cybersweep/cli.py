"""Command line interface for CyberSweep.

Examples::

    cybersweep check
    cybersweep discover 192.168.1.0/24
    cybersweep scan 192.168.1.10 -p common
    cybersweep scan 192.168.1.0/24 -p 22,80,443,3389 --cve --html report.html --csv out.csv
    cybersweep report 3 --format html --min-severity HIGH -o report.html
    cybersweep history
    cybersweep gui
"""
...
from cybersweep import __version__, config, discovery, report, storage, utils
from cybersweep.engine import ScanEngine, environment_report
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="cybersweep",
        description="CyberSweep - network inspector: host discovery, port scanning, "
                    "service identification and vulnerability lookup.",
        epilog=config.LEGAL_NOTICE,
    )
    parser.add_argument("--version", action="version", version=f"cybersweep {__version__}")
    ...
    parser.add_argument("--db", metavar="PATH", help="SQLite database path "
                        "(default: ~/.cybersweep/cybersweep.db or $CYBERSWEEP_HOME)")
    ...
    # check ---------------------------------------------------------------
    sub.add_parser("check", help="show environment information (nmap, python-nmap, local network)")
    ...
    p_scan.add_argument("target", help="IP, range (10.0.0.1-50), CIDR (10.0.0.0/24) or hostname; "
    ...
    p_rep.add_argument("-f", "--format", choices=["text", "markdown", "html", "csv", "json"],
                       default="text")
...
def cmd_discover(args) -> int:
    target = args.target or utils.local_network_cidr()
...
def cmd_scan(args) -> int:
    ...
    if opts.method == "nmap" and not (utils.nmap_available() and utils.python_nmap_available()):
        print("error: --method nmap requested but nmap/python-nmap is not available. "
              "Run 'cybersweep check' for install hints.", file=sys.stderr)
        return 2
    progress = ConsoleProgress(not args.quiet)
    ...
    if result.scan_id:
        print(f"Saved as scan #{result.scan_id}. Re-render later with: cybersweep report {result.scan_id}")
...
def cmd_report(args) -> int:
    with _open_db(args) as db:
        ...
        if scan_id is None:
            print("No scans stored yet. Run 'cybersweep scan <target>' first.", file=sys.stderr)
            return 1
    ...
    if args.format == "csv":
        target = args.output or f"cybersweep_scan_{scan_id}.csv"
...
def cmd_gui(args) -> int:
    try:
        from cybersweep.gui import launch
...
COMMANDS = {
    "check": cmd_check, "discover": cmd_discover, "scan": cmd_scan, "report": cmd_report,
